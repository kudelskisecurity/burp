...
class Connector:
    ...
    def post(self,
             ...
             file: Optional[bytes]) -> requests.Response:
        url = self.__get_url(path)
...
class Base:
    ...
    @staticmethod
    def __check_status_code(status_code: Tuple[int, ...],
    ...
    def _get(self,
             ...
             path: Optional[str] = None  # ignore: E251
             ) -> requests.Response:
        rpath = self.__resolve_path(path)
        ret = self.__connector.get(rpath)
        return self.__check_status_code(status_code, ret)

    def _post(self,
              status_code: Tuple[int, ...],
              json: Optional[Mapping[str, Any]] = None,
              path: Optional[str] = None,
              file: Optional[bytes] = None) -> requests.Response:
        rpath = self.__resolve_path(path)
        ret = self.__connector.post(rpath, json, file=file)
        return self.__check_status_code(status_code, ret)
    ...
    def _delete(self,
        ...
        rpath = self.__resolve_path(path)
        ret = self.__connector.delete(rpath)
        return self.__check_status_code(status_code, ret)
